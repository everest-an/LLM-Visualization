...
export function initProgramState(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData): IProgramState {
    ...
    return {
        ...
        examples: [{
            name: 'MT-LNN',
            enabled: true,
            shape: { B: 1, T: 6, C: 48, vocabSize: 3, nBlocks: 2, A: 16, nHeads: 3, dHead: 16 } as IModelShape,
            layout: createMTLNNLayout(DEMO_MTLNN_CONFIG, new Vec3()) as any,
            offset: delta.mul(-12),
            modelCardOffset: new Vec3(),
            blockRender: initBlockRender(render?.ctx ?? null),
            camera: makeCamera(new Vec3(-119753, 0, -540), new Vec3(235, 18, 20.0)),
        }, {
            name: 'GPT-2 (small)',
            enabled: true,
