...
export function initWalkthrough() {
    return {
        ...
        phaseList: [{
            ...
            title: 'MTLNN Architecture',
            phases: [
                { id: Phase.MTLNN_Intro, title: 'Overview' },
                { id: Phase.MTLNN_Tokens, title: 'Embeddings' },
                { id: Phase.MTLNN_Architecture, title: 'MT-DL Core' },
                { id: Phase.MTLNN_Gates, title: 'Gates (kappa & map)' },
                { id: Phase.MTLNN_Gwtb, title: 'GWTB' },
                { id: Phase.MTLNN_Wm, title: 'Working Memory' },
                { id: Phase.MTLNN_Output, title: 'LM Head / Output' },
            ],
        }] as IPhaseGroup[],
    };
}
...
export { Phase, PhaseGroup } from './PhaseEnum';
import { Phase, PhaseGroup } from './PhaseEnum';

export function phaseToGroup(wt: IWalkthrough) {
    let group = wt.phaseList.find(g => g.phases.find(p => p.id === wt.phase));
    if (!group) return wt.phaseList[0];
    return wt.phaseList.find(g => g.phases.find(p => p.id === wt.phase))!;
}


import { walkthroughMTLNN } from "./Walkthrough10_MTLNN";
...
export function runWalkthrough(state: IProgramState, view: IRenderView) {
    ...
    } else if (groupId === PhaseGroup.MTLNN) {
        walkthroughMTLNN(wtArgs);
    }
