...
import { phaseToGroup, IWalkthrough, Phase, PhaseGroup } from './walkthrough/Walkthrough';
...
import { MtLnnToc } from './components/MtLnnToc';
...
export const Commentary: React.FC = () => {
    ...
    // dynamically switch phaseList
    if (progState.currExampleId === 0) {
        progState.walkthrough.phaseList = [{
            groupId: PhaseGroup.MTLNN,
            title: 'MT-LNN',
            phases: [
                { id: Phase.MTLNN_Intro, title: 'Introduction' },
                { id: Phase.MTLNN_Tokens, title: 'Tokens & Embedding' },
                { id: Phase.MTLNN_Architecture, title: 'MT-DL' },
                { id: Phase.MTLNN_Gates, title: 'Scan & Gates' },
                { id: Phase.MTLNN_Gwtb, title: 'GWTB' },
                { id: Phase.MTLNN_Wm, title: 'WM & Orch-OR' },
                { id: Phase.MTLNN_Output, title: 'Output Logits' },
            ],
        }];
        if (!progState.walkthrough.phaseList.some(g => g.phases.some(p => p.id === progState.walkthrough.phase))) {
            progState.walkthrough.phase = Phase.MTLNN_Intro;
        }
    } else {
        // Assume default
        progState.walkthrough.phaseList = [{
        ...
        if (!progState.walkthrough.phaseList.some(g => g.phases.some(p => p.id === progState.walkthrough.phase))) {
            progState.walkthrough.phase = Phase.Intro_Intro;
        }
    }
        ...
        <div className={s.walkthroughViewport}>
            <div className={s.walkthroughText} tabIndex={0} onKeyDownCapture={handleKeyDown}>
                <div className={s.tocBackground}>
                    {progState.currExampleId !== 0 ? <TocDiagram activePhase={phase.id} /> : <MtLnnToc activePhase={phase.id} />}
                </div>
